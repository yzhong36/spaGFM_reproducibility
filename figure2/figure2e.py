...
# Original filename: draw_prediction_outline.py
# Last modified: 2026-08-12 15:28:25  (git: n/a - no git repository anywhere under graph_foundation_model/)
# Input data: /fs/ess/PAS1475/Fei/data/spaGFM/TLS_data/KC/test/pick/new_model_results/GSM5924038_ffpe_c_36_corrected.h5ad, obs['317M_predict'] (0 = TLS, 2 = Non-TLS, n_TLS = 271)
# Output: TLS_prediction_outline_20260812/panel_e_prediction_GSM5924038_ffpe_c_36.{png,pdf,svg}
# Match evidence: the on-disk panel_e_prediction_GSM5924038_ffpe_c_36.png is the panel in the slide - same H&E crop, same burnt-orange (#b15929) spots, same traced region outlines, same small upper-right blob plus the large ragged lower-right region. The script writes the literal title 'Prediction' and the bold panel letter 'e' that appear in the slide. Run log: TLS_prediction_outline_20260812/logs/outline_6814314.log ("5 regions with >= 2 spots (271 spots inside them)").
# Other candidates considered: TLS_figure_317M_annotated/redo_annotation_panels.py and TLS_figure_317M/redo_sample_panels_all_corrected.py draw the same spots but as flat fills with a legend and NO region outlines, so neither produces this panel. panel_e_prediction_spaGFM_* (same script, spaGFM_binary_predict, 334 spots / 6 regions) is a comparison panel and does NOT match the slide.
