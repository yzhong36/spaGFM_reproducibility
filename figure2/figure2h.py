...
# Original filename: redo_figure_D.py   (SAME script as figure2g - one script, two panels)
...
# Output: TLS_figure_317M/figure_D_mean.{png,pdf,svg}  --  RIGHT subplot ("B. Neighborhood Purity for TLS-Predicted Spots")
# Match evidence: bar lengths in the slide read 0.86 / 0.63 / 0.245 / 0.217 / 0.19 / 0 / 0, matching figure_D_mean's printed values 0.860 (Ground Truth), 0.630 (spaGFM), 0.245 (finetune-scGPT), 0.217 (scGPT), 0.190 (NOVAE), n/a (NicheCompass), 0.000 (scGPT-spatial). Same row order and fill colours as figure2g; axis label "Neighborhood purity". The single-sample figure_D.{png,pdf,svg} does NOT match (0.879 / 0.431 / 0.324 / 0.286 / 0.264).
# Slide edits made outside this script: "Scanpy" row (0.046) deleted; "Ground Truth" -> "Annotation", "finetune-scGPT" -> "Finetuned scGPT", "NOVAE" -> "Novae"; value annotations and the red "Random (1/6)" reference line removed.
# Note: identical file to figure2g.py. Running it once produces both panels.
# Other candidates considered: same as figure2g - TLS_figure/redo_figure_D.py (older, pre-317M) and the tls_validation_figures* multi-panel sheets.
